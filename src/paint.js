...
let ctx = canvas.getContext('2d', { willReadFrequently: true });let drawing = false;
let paintOpen = false;
let tool = 'pen';
let undoStack = [];
...
function makeDraggable(element, handle) {
    ...
    function dragMouseDown(e) {
        if (e.target.tagName === 'INPUT' || e.target.tagName === 'BUTTON' || e.target.classList.contains('c-btn')) return;        e.preventDefault();
        pos3 = e.clientX;
        pos4 = e.clientY;
...
function toggleMathTool(toolId) {
    ...
    if (getComputedStyle(el).display === 'none') {
        ...
        el.style.top = "150px";
        el.style.left = "250px";        el.dataset.rotation = 0;
...
function toolScale(e, toolId) {
    ...
    s = (s >= 2.0) ? 0.5 : s + 0.2;    el.dataset.scale = s;
    updateToolTransform(el);
}
...
function saveState() {
    if (undoStack.length >= 25) undoStack.shift();    undoStack.push(canvas.toDataURL());
}
